    ...
    for Api
{
    ...
    fn blocking_initialize_product(&self, name: String, description: String) -> () {
        let result = self
            ...
            .invoke_and_await(
                ...
                &[WitValue::builder().string(&name), WitValue::builder().string(&description)],
            )
            .expect(&format!(
    ...
    fn blocking_get(&self) -> Option<crate::bindings::golem::shopping_product::api::Product> {
        let result = self.rpc.invoke_and_await("golem:shopping-product/api.{get}", &[]).expect(
            &format!("Failed to invoke-and-await remote {}", "golem:shopping-product/api.{get}"),
        );
    ...
    fn get(
        ...
        let result = self.rpc.async_invoke_and_await("golem:shopping-product/api.{get}", &[]);
