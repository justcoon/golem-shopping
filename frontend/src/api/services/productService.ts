...
import {getProductPricing, getBatchPricing, getBestPrice, type Pricing, SalePricingItem} from './pricingService';
...
const enhanceWithPricing = async (product: Product): Promise<Product> => {
  try {
    ...
    console.error(`Error enhancing product ${product['product-id']} with pricing:`, error);
...
export const getProductById = async (productId: string, includePricing = true): Promise<Product> => {
  try {
...
export const getProductsByIds = async (productIds: string[]): Promise<Record<string, Product>> => {
  try {
...
export const getProductOriginalPrice = (product: Product): string =>  {
    if (product.pricing && product.pricing?.['list-prices'].length > 0) {
        const minListPrice = Math.min(...product.pricing['list-prices'].map((p: any) => p.price));
...
export const isProductOnSale = (product: Product): boolean =>  {
    ...
    const bestSalePrice = Math.min(...product.pricing['sale-prices']
    ...
    const minListPrice = product.pricing?.['list-prices']?.length
        ? Math.min(...product.pricing['list-prices'].map((p: any) => p.price))
        : Infinity;
