...
export const getUserOrders = async (userId: string, orderIds: string[] = []): Promise<Order[]> => {
    try {
        ...
        const orderPromises = orderIds.map(orderId => getOrder(orderId).catch(error => {
            console.error(`Error fetching order ${orderId}:`, error);
            return null;
        }));
...
export const updateOrderBillingAddress = async (orderId: string, address: Address): Promise<Order> => {
    try {
        const response = await apiClient.put(`/v1/order/${orderId}/billing-address`, address);
        return response.data;
    } catch (error) {
        console.error(`Error updating billing address for order ${orderId}:`, error);
        throw error;
    }
};

export const updateOrderShippingAddress = async (orderId: string, address: Address): Promise<Order> => {
    try {
        const response = await apiClient.put(`/v1/order/${orderId}/shipping-address`, address);
        return response.data;
    } catch (error) {
