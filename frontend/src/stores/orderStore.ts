...
export const useOrderStore = defineStore('orders', () => {
    ...
    const fetchOrder = async (orderId: string) => {
        ...
        try {
            ...
            await Promise.all(order.items.map(async (item: OrderItem) => {
                try {
                    const product = await productStore.fetchProduct(item["product-id"]);
    ...
    const updateEmail = async (orderId: string, email: string) => {
        try {
            ...
            const orderIndex = orders.value.findIndex(o => o["order-id"] === orderId);
            if (orderIndex !== -1) {
    ...
    const updateShippingAddress = async (orderId: string, address: Address) => {
        ...
        try {
            const updatedOrder = await updateOrderShippingAddressApi(orderId, address);
